...
from smaller.tasks import send_verification_email
...
def history(request):
    return render(request,"history.jinja")
# dwitter/views.py
def create_short_post(request):
    if request.method == "POST":
        ...
        link.short_link = get_random_string(15)
...
def get_all_links(request):
    links = Link.objects.all().filter(user_id = request.user.id)
...
def delete_link(request):
    ...
    body_unicode = request.body.decode('utf-8')
    body = json.loads(body_unicode)
    Link.objects.get(Q(user_id = request.user.id), Q(id = body["id"])).delete()
    return HttpResponse()
...
def update_link(request):
    ...
    body_unicode = request.body.decode('utf-8')
    body = json.loads(body_unicode)
    link = Link.objects.get(Q(user_id = request.user.id), Q(id = body["id"]))
...
def redirect_to_original(request):
    redirect_link = ""
    try:
        redirect_link = Link.objects.get(short_link = request.path[1:]).original_link
    except:
        redirect_link = "index"
